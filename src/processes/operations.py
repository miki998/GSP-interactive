...
def compute_basis(L:np.ndarray, method:str="eig", tol:float=1e-13, verbose:bool=True):
    """
    Computes basis for transform matrix supporting different methods:

    - eig: Eigendecomposition
    - jord: Jordan decomposition 
    - ortho: Orthogonality optimization
    ...
    if method == "eig":
        ...
        if np.all(np.abs(eigval - 1) < 1e-10):
            ...
            V = eigval[np.argsort(np.abs(eigval))]
            U = eigvect[:, np.argsort(np.abs(eigval))]

    elif method == "jord":
        symbolicL = Matrix(L)
        try:
            V, U = symbolicL.jordan_form()
            U = np.array(U, dtype=float)
            V = np.array(V, dtype=float)
        except:
            print("Not supporting all type of matrices")
            return None, None
...
def select_undirectededge_CN(mat: np.ndarray):
    """
...
###################################
#### Graph Spectral Operations ####
###################################

def hilbert_filter(V:np.ndarray):
    """
    Compute filter for Hilbert Transform i.e In GFT space apply rotation 90
    ...
    cond = V.imag
    filter_h = (
        np.array((cond < 0), dtype=float) * 1j + np.array((cond > 0), dtype=float) * -1j
    )
...
def signal_seqshift(signal:np.ndarray, S:np.ndarray, nstep:int):
    """
    Compute sequential shifts of a signal vector using a shift matrix.

    Parameters
    ----------
    signal : ndarray
        Input signal vector to be shifted.
    S : ndarray
        Shift matrix.
    nstep : int  
    ...
    -------
    ret : list
        List containing the input signal vector and the sequentially shifted versions.
    """

    tmp_sig = deepcopy(signal)
    ret = [tmp_sig]
    for _ in range(nstep):
        tmp_sig = np.matmul(S, tmp_sig)
        ret.append(tmp_sig)
    return ret
