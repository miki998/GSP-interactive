...
def plot_graph(G:nx.Graph, nd_values:np.ndarray, pos:Optional[dict]=None, cmap:Optional[colors.Colormap]=None, 
               ...
               colorbar:bool=False, node_shape:str="o", nodetype:bool="size", alpha:float=1.0):
    """
    ...
    node_shape : str, optional
        Matplotlib node shape specifier
    ...
    if nodetype == "color":
        nx.draw(G,arrows=True,node_color=nd_values,alpha=alpha,
                pos=pos,ax=ax,cmap=cmap,node_shape=node_shape)
    elif nodetype == "size":
        nx.draw(G,arrows=True,node_size=node_values,node_color=nd_color,alpha=alpha,
                pos=pos,ax=ax,cmap=cmap,node_shape=node_shape)
    else:
...
def prep_transform(A:np.ndarray, gso:str="adj", composite:bool=True, verbose:bool=False):
    ...
    if gso == "adj":
        ...
        L = compute_directed_laplacian(A)

    U, V = compute_basis(L, verbose=verbose)
    ...
    if composite:
        S, I = compute_basis(
            hermitian(np.diag(cleanV)) @ hermitian(U) @ U @ np.diag(cleanV)
        )
        J = np.diag(I)
...
def vis_graph(A:np.ndarray, U:Optional[np.ndarray]=None, diracsig:Optional[list]=None, 
    ...
    if U is None:
        ...
        sig = np.zeros(len(A))
        if diracsig is None:
            sig = np.abs(np.random.random(len(A)))
        else:
